module "wellmum_api" {
  ...
  db_name                   = "wellmum_api_prod"
  db_username               = var.db_username
  db_password               = var.db_password
  serverless_min_capacity   = 0.5    # 0.5 ACU minimum pour prod
  serverless_max_capacity   = 1     # 1 ACU maximum pour prod
  ...
  aws_s3_bucket_name     = "wellmum-prod-storage"
